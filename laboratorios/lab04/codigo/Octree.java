...
public class Octree
{
    ...
    public void octree(LinkedList<Bee> abejas,ArrayList<Double> mins,double midD,double midW,double midH) {
        ...
        for (int i=0; i<abejas.size(); ++i) {
            ...
            int espacio = hashing(abeja,mins);
            octree.get(espacio).addFirst(abeja);
        }

        double Hip = Math.sqrt(Math.pow((midD)*111325,2) + Math.pow((midW)*111325,2)); //1 grado = 111325,2 metros
        
        double diagonal = Math.sqrt(Math.pow(Hip,2) + Math.pow((midH),2)); 
